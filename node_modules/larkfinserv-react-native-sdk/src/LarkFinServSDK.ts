...
class LarkFinServSDK {
  ...
  public async initialize(config: PartnerConfig): Promise<void> {
    try {
      ...
      const userId =
        response.data.user?.id || response.data.userId || this.config.userId;
   ...
   */
  public async openEligibilityCheck(mode: SDKMode): Promise<void> {
    if (!this.iframeUrl) {
   ...
   */
  public off(event: SDKEvent['type']): void {
   ...
   */
  public createWebViewMessage(data: Record<string, unknown>): string {
  ...
  private async submitConsentData(): Promise<void> {
    ...
    try {
      ...
      const response = await axios.post(
        `${getSDKApiUrl(this.config.environment)}/loan-sdk/consents/submit`,
        consentPayload,
        {
          headers: {
            ...DEFAULT_HEADERS,

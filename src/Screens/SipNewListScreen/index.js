  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${baseURL}${endpoints.SIP_PURCHASE_BATCH}`,
        body,
        {
          headers: {
      ...
      if (res?.status === true && res?.data?.results?.length > 0) {
        ...
        navigation.navigate('SIPSecondForm', {
          ids,
            ...
            onPress: async () => {
              ...
              try {
                const response = await axios.delete(
                  `${baseURL}${endpoints.CART_ORDERS}${ClientId}/all?trx_type=sip`
                );
                const res = response.data?.response;
    ...
    <View style={CommonStyles.container}>
      ...
      <View style={styles.scrollContent}>
        <LumpsumListContainer
          ...
          onAmountSelected={(amount) => setSelectedAmount(amount)}
        />
        <AddReferalDetails />
        <AddMoreLumpsum
...
export default SipNewListScreen;

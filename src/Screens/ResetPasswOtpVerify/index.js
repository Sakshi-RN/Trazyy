...
const LoginOtpVerify = ({ route }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setLoading(false);
      if (res.ok && json?.status && json?.statusCode === "0") {
        if (json.response?.status) {
          navigation.reset({
            index: 0,
            routes: [{ name: "ChangePassword", params: { email: json.response.email } }],
          });
        } else {
          setMessage(json.response?.message || "OTP verification failed");
        }
      } else {
        setMessage(json?.statusMessage || "Something went wrong");
      }
    ...
    try {
      ...
      if (response.ok && result?.status && result?.statusCode === "0") {
        if (result.response?.status) {
          setMessage(result.response?.message || "OTP resent successfully");
          setTimer(180);
